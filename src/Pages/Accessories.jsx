  ...
  const handleSaveProduct = async () => {
    try {
      if (editProductId) {
        // Edit Product
        await axios.put(
          `http://localhost:5000/api/accessories/${editProductId}`,
          newProduct
        );
      } else {
        // Add New Product
        await axios.post("http://localhost:5000/api/accessories", newProduct);
      }
      setNewProduct({
        productName: "",
        brand: "",
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-6 rounded shadow-lg w-full max-w-md h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4">
              {showEditModal ? "Edit Accessory" : "Add Accessory"}
            </h2>
