...
const Coolers = () => {
  ...
  const handleSaveProduct = async () => {
    try {
      if (editProductId) {
        // Edit Product
        await axios.put(
          `http://localhost:5000/api/coolers/${editProductId}`,
          newProduct
        );
      } else {
        // Add New Product
        await axios.post("http://localhost:5000/api/coolers", newProduct);
      }
      setNewProduct({
        productName: "",
        brand: "",
  ...
  const handleDeleteProduct = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/coolers/${id}`);
      fetchProducts();
    } catch (error) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-6 rounded shadow-lg w-full max-w-md h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4">
              {showEditModal ? "Edit Cooler" : "Add Cooler"}
            </h2>
        ...
        <table className="min-w-full table-auto">
          <thead className="bg-blue-600 text-white">
            <tr>
              ...
              <th className="px-4 py-2">Cooling Capacity</th>
                ...
                product.variants.map((variant, idx) => (
                  <tr key={variant._id}>
                    ...
                    {idx === 0 && (
                      <td
                        rowSpan={product.variants.length}
                        className="border px-4 py-2 flex gap-2 justify-center"
                      >
                        <button
